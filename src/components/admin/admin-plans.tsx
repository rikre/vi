...
export function AdminPlans({ state }: { state: AdminState }) {
  ...
  const closePlan = () => {
    ...
    const original = state.plans.find((item) => item.id === planDraft?.id);
    if (planDraft && JSON.stringify(planDraft) !== JSON.stringify(original) && !window.confirm("放弃未保存的套餐配置？")) return;
    setPlanDraft(null);
    setSaveError(null);
  };
  const closeTier = () => {
    if (saveLock.current) return;
    const original = state.rechargeTiers.find((item) => item.id === tierDraft?.id);
    if (tierDraft && JSON.stringify(tierDraft) !== JSON.stringify(original) && !window.confirm("放弃未保存的充值档位？")) return;
    setTierDraft(null);
    setSaveError(null);
  };
...
function FormActions({ onCancel, saving }: { onCancel: () => void; saving: boolean }) { return <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onCancel} className={ADMIN_SECONDARY_BUTTON}>取消</button><button type="submit" className={ADMIN_PRIMARY_BUTTON}>{saving ? "保存中…" : "保存配置"}</button></div>; }
